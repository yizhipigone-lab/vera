...
### 4.2 减仓语境(FUSED → gross,★审计 H1:只报告不联仓位)

**★ 审计 H1**:撞 CLAUDE.md 铁律 1(宏观/地缘只报告不联仓位)。默认按铁律 1:**regime 减仓语境只生成报告提示,不自动改仓位**。`PipelineResult` 加 `regime_gross_suggested`(建议字段,非执行)。若要 regime 自动减仓,**需用户显式确认 regime 不属宏观**(待用户拍板)。
