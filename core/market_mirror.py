...
def mirror(top_n: int = 5) -> dict:
    ...
    sh = mpio._index_series("000001.SH")
    ...
    def _n_eff(k: int) -> float | None:
        ...
        """
        return mpio._f(len(band) / float(k), 1) if k > 0 else None

    s20, s60 = pd.Series(f20), pd.Series(f60)
    summary = {
        "n": len(band),
        "n_picks": len(picks),
        "fwd_20_median": mpio._f(s20.median(), 2) if f20 else None,
        "fwd_20_mean": mpio._f(s20.mean(), 2) if f20 else None,
        "fwd_20_q25": mpio._f(s20.quantile(0.25), 2) if f20 else None,
        "fwd_20_q75": mpio._f(s20.quantile(0.75), 2) if f20 else None,
        "fwd_20_up_ratio": mpio._f(sum(1 for x in f20 if x > 0) / len(f20) * 100, 0) if f20 else None,
        "fwd_60_median": mpio._f(s60.median(), 2) if f60 else None,
        "fwd_60_up_ratio": mpio._f(sum(1 for x in f60 if x > 0) / len(f60) * 100, 0) if f60 else None,
        "n_eff_20": _n_eff(20),
        "n_eff_60": _n_eff(60),
    }
    years = mpio._year_breakdown(band, "fwd_20_hs300_pct")
    ...
    if total:
        ...
        if top["n"] / total > YEAR_DOMINANCE:
            ...
            warnings.append(YEAR_DOMINANCE_WARNING.format(
                ...
                share=mpio._rat(top["n"] / total * 100)))
