# -*- coding: utf-8 -*-
"""公式农场判定口径 — **单一真相源** (2026-09-11 收口; 2026-09-22 年化线下调)。

**2026-09-22 用户拍板: 粗扫达标线年化下限由 15% 下调为 10%**, 现行口径:

    **年化 ≥ 10% 且 |最大回撤| ≤ 15% 且 笔数 ≥ 20**

同日已按新口径重算 `data/formula_farm/archive.json` 全部历史判定
(只重算判定, 不回撤重跑回测; 旧档案备份在 archive_backup_20260922_阈值下调前.json)。

历史沿革 (全部有出处, 不是拍脑袋):

- **原 15%**: QUANTQQ 系扫描器里早就写着"用户拍板: 年化不低于 15%"
...
- **2026-09-10 已有预演**: 用户在聚焦三公式研究里把门槛临时降到 ≥10%
  (`docs/2026-09-10_公式农场聚焦三公式_最终结论报告.md:4`), 09-22 正式落为农场口径。
...
#: 达标: 年化下限 (2026-09-22 用户拍板: 0.15 → 0.10)
TARGET_ANN = 0.10
#: 达标: 最大回撤绝对值上限
TARGET_MAXDD = 0.15
#: 最小样本门槛 (笔数), 不足则记「样本不足」
