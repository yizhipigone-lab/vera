...
class BenchmarkComparator:
    ...
    def fetch_and_compare(
        ...
        for idx_name in self.index_names:
            ...
            comparison = self._align(equity_curve, index_df, idx_name)
    ...
    def _fetch_index(
        ...
        df = DataFetcher.get_index_data(
            index_name, start_time, end_time, dividend_type="none",
        )
        return df
    ...
    def _align(
        ...
        # 计算超额收益统计
        total_excess = comparison["excess_return"].sum()
        annual_excess = total_excess / len(comparison) * 252
