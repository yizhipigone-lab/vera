        ...
        while p < pos_count:
            ...
            if triggered >= 0:
                ...
                if triggered == 3:
                    ...
                    sell_price = ep * (1.0 + cost_stop_threshold)
                    actual_ret = cost_stop_threshold
                elif triggered == 5:
                    # 阶梯止盈【简化模式】：使用ladder_price执行
                    tp_profit = 0.0
                    mask = pos_ladder_done[p]
                    for li in range(n_ladder):
                        if (mask >> li) & 1 and hi_pp >= ladder_profits[li]:
                ...
                if triggered == 5:
                    ...
                    if sell_ratio < 1.0:
                        ...
                        if sell_sh < total_sh:
                            gross = sell_sh * sell_price * (1.0 - commission)
                            cash += gross
                ...
                # 全卖（所有非部分卖出场景）
                gross = total_sh * sell_price * (1.0 - commission)
                cash += gross
                if trade_count < max_trades:
                    trades[trade_count, 0] = float(ci)
                    trades[trade_count, 1] = float(pos_entry_idx[p])
...
class BacktestEngine:
    ...
    def run(self, selections, start_time="", end_time="", stop_config=None):
        ...
        kline = DataFetcher.get_kline(codes, start_time, end_time, dividend_type="front", period=self.period)
        ...
        if high_df is not None:
            ...
            # 确保close和high列一致
            final_cols = sorted(set(cols) & set(high_df.columns))
            close = close[final_cols]
            entries = entries[final_cols]
            high_np = high_df[final_cols].values.astype(np.float64)
            cols = final_cols
        if low_df is not None:
            cl_cols = sorted(set(cols) & set(low_df.columns))
            low_df = low_df.reindex(index=close.index, columns=cl_cols).ffill().bfill()
            final_cols = sorted(set(cols) & set(low_df.columns))
            close = close[final_cols]
            entries = entries[final_cols]
            if high_np is not None:
                high_np = high_df[final_cols].values.astype(np.float64)
            low_np = low_df[final_cols].values.astype(np.float64)
        ...
        metrics = MetricsCalculator.compute_all(equity_curve, trades_df, self.initial_capital,
                                                  periods_per_year=self.bars_per_day * 252)
    ...
    def _filter_limit_up(self, entries, close):
        ...
        for col in entries.columns:
            ...
            elif 'ST' in col_str or '*ST' in col_str: limit_ratio = 0.05
