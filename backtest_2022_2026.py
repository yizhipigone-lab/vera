...
def run_one(close, entries, cost_th, trail_act, trail_dd, lp_v, lr_v, time_d, cap, max_pct):
    ...
    _sc = {
        "priority": "stop_first",
        "cost_stop": {"enabled": True, "threshold": float(cost_th)},
        "trailing_stop": {"enabled": True, "activation": float(trail_act), "drawdown": float(trail_dd)},
        "ladder_tp": {"enabled": True, "levels": []},
        "time_stop": {"enabled": te, "max_hold_days": int(time_d) if te else 999},
