# -*- coding: utf-8 -*-
"""zza500_long_optimize.py
========================
中证A500 长期寻优 (2006-01-01 ~ 2026-07-31, 约 20.5 年), 门槛年化 > 18.88%。

用户指令 (2026-08-11):
  - 区间从 2006 起 (穿越 2007 牛市 / 2008 股灾 / 2015 杠杆牛+股灾 / 2021 核心资产牛)
  - 门槛: 年化 > 18.88% (实盘可达目标, 已含乐观偏差打折)
  - 买入口径: 尾盘选股尾盘买入 (≈ T 收盘价, 回测口径, 非 T+1)

⚠️ 幸存者偏差声明: 中证A500 指数 2024 年才发布, 用今日 500 只成分股回测到 2006 年,
   20 年间退市/ST/被剔除股全不可见 → 偏高 (比 7.5 年区间严重得多)。数字仅供参考,
   需沪深300历史成分对照挤水分。

用法: python -X utf8 research/zza500_long_optimize.py
"""
...
# 用户指令参数 (本地定义, 不继承 auto_strategy_optimizer 的 2019 起点/25% 门槛)
LONG_START, LONG_END = "20060101", "20260731"
TARGET_ANN = 0.1888            # 年化 > 18.88%
UTYPE = "28"
POOL_NAME = "中证A500"
...
def main():
    print("=" * 90)
    print(f"  中证A500 长期寻优 | {LONG_START}~{LONG_END} (约 20.5 年) | 日线 | 尾盘买入口径")
    ...
    logger.info("股票池 %s (type=%s) -> %d 只 (今日快照, 20年幸存者偏差严重)", POOL_NAME, UTYPE, len(codes))
    ...
    for fkey, fparams, fname in FACTOR_GRID:
        ...
        for cost, tact, tdd, tdays, ladder, ename in EXIT_GRID:
            ...
            try:
                ...
                # 20年区间, 直接调 engine (run_backtest 写死了 2019 起点不能用)
                ...
                logger.info("%-44s | 信号=%-7d 年化=%6.2f%% 回撤=%6.2f%% 夏普=%5.2f 交易=%-6d 胜率=%5.1f%% %s",
    ...
    if rows:
        ...
        out = os.path.join(RUN_DIR, "zza500_long_results.csv")
    ...
    print("=" * 90)
    logger.info("长期寻优完成, 总耗时 %.0fs", time.time() - t_start)
