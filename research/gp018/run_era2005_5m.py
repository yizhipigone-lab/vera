"""GUPIAO_018 2005-2026 真5分钟回测 (止损优先, -12% / 6%+15% / 3.5%+1% / 20天)。

MAXBUY 环境变量控制单票上限 (默认 20000)。
"""
...
BT = {
    ...
    "position_sizing": {"max_positions": 999, "min_buy_amount": 2000.0,
                        "max_buy_amount": float(os.environ.get("MAXBUY", "20000")),
                        "lot_size": 100, "min_lots": 1},
}

START, END = "20050101", "20260801"
TAG = "era2005_5m_" + os.environ.get("SUFFIX", "mb2w")

sel = pd.read_parquet("research/gp018/results/server_gp018_2005_2026.parquet")
sel["select_date"] = pd.to_datetime(sel["select_date"])
print(f"[INFO] {TAG} 信号 {len(sel)} 条 / {sel['stock_code'].nunique()} 只", flush=True)
t0 = time.time()
...
with open(f"research/gp018/results/{TAG}.json", "w", encoding="utf-8") as f:
    json.dump({"range": [START, END], "stop": STOP, "bt": BT, "metrics": m},
