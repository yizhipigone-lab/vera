...
SIG_FILES = ["research/gp018/results/server_gp018_2005_2026.parquet"]


def stop_variant(name):
    st = copy.deepcopy(BASE_STOP)
    if name == "champ1":      # 关止损 + 3.5%/1% + 阶梯6%/15%卖30% + 12天
        st["cost_stop"] = {"enabled": False, "threshold": -0.06}
        st["ladder_tp"] = {"enabled": True, "levels": [
            {"profit": 0.06, "sell_ratio": 0.30}, {"profit": 0.15, "sell_ratio": 0.30}]}
        return st
    if name == "champ2":      # 关止损 + 3.5%/1% + 无阶梯 + 12天
        st["cost_stop"] = {"enabled": False, "threshold": -0.06}
        st["ladder_tp"] = {"enabled": False, "levels": []}
        return st
    if name == "champ3r":     # champ3 + real 语义(条件单: 本bar创新高不触发/跳空按开盘/触线按线价)
        st["cost_stop"] = {"enabled": True, "threshold": -0.12}
        st["ladder_tp"] = {"enabled": True, "levels": [
            {"profit": 0.06, "sell_ratio": 0.30}, {"profit": 0.15, "sell_ratio": 0.30}]}
        st["trailing_stop"]["confirm"] = "real"
        return st
    if name == "champ3s":     # champ3 + simple 语义(5M碰线按bar收盘/1D收盘价判定, 无阶梯休息)
        st["cost_stop"] = {"enabled": True, "threshold": -0.12}
        st["ladder_tp"] = {"enabled": True, "levels": [
            {"profit": 0.06, "sell_ratio": 0.30}, {"profit": 0.15, "sell_ratio": 0.30}]}
        st["trailing_stop"]["confirm"] = "simple"
        return st
    if name == "champ3c":     # champ3 + 移动止盈收盘价确认(尾盘判定,收盘价成交)
        st["cost_stop"] = {"enabled": True, "threshold": -0.12}
        st["ladder_tp"] = {"enabled": True, "levels": [
            {"profit": 0.06, "sell_ratio": 0.30}, {"profit": 0.15, "sell_ratio": 0.30}]}
        st["trailing_stop"]["confirm"] = "close"
        return st
    if name == "champ3":      # -12% + 3.5%/1% + 阶梯6%/15%卖30% + 12天
        st["cost_stop"] = {"enabled": True, "threshold": -0.12}
        st["ladder_tp"] = {"enabled": True, "levels": [
            {"profit": 0.06, "sell_ratio": 0.30}, {"profit": 0.15, "sell_ratio": 0.30}]}
        return st
