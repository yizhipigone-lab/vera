# -*- coding: utf-8 -*-
"""zz500_long_optimize.py
========================
中证500 长期寻优 (2006-01-01 ~ 2026-07-31, 约 20.5 年), 门槛年化 > 18.88%。

为何测中证500 (2026-08-11 决策, 三池最后一块):
  - 中证A500 20年天花板17.96% (虚高, 段2腰斩-52.85%) ❌
  - 沪深300 20年天花板14.43% (诚实, 但alpha衰减太低) ❌
  - 中证500: 中盘甜区 (同A500档, MA金叉最如鱼得水) + 2007发布 (成分股比A500老,
    数据相对全) + 7.5年=28.18% (介于A500 30.58% / 沪深300 18.81%)
  - 按沪深300衰减率23%推算, 中证500 20年可能18-22%, 有达标希望。

6 因子 × 3 出场 = 18 组合, 全区间排序 + 达标标记。

用法: python -X utf8 research/zz500_long_optimize.py
"""
...
def main():
    print("=" * 90)
    print(f"  中证500 长期寻优 | {LONG_START}~{LONG_END} (约 20.5 年) | 日线 | 尾盘买入口径")
    print(f"  参数: 单票{MAX_AMT//1000/10:.0f}万 + {MAX_POS}仓 + 冷却{COOLDOWN}天 | 门槛: 年化>{TARGET_ANN:.2%} (回撤<={MAX_DD:.0%}作参考)")
    print(f"  中证500: 中盘甜区 + 2007发布 + 7.5年基线28.18% (三池最有希望达标)")
    print("=" * 90)
    ...
    if rows:
        ...
        out = os.path.join(RUN_DIR, "zz500_long_results.csv")
    ...
    print("=" * 90)
    logger.info("中证500长期寻优完成, 总耗时 %.0fs", time.time() - t_start)
