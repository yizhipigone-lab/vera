# -*- coding: utf-8 -*-
"""自动寻优第 87 轮策略 (self-contained, 可独立运行): 趋势回踩10/60
生成时间: 2026-08-11 07:24
因子: double_ma_pullback  入场参数: {"fast": 10, "slow": 60}
出场: 硬止损=-0.1 移动止盈激活=0.08/回撤=0.03 (条件单语义) 时间=20天 阶梯=None
仓位: 最多持 10 只, 单票 5000~30000元
手续费/滑点: 0.0003 / 0.001
区间: 20190101 ~ 20260731  股票池: hs300 (type=23, 剔除ST)
绩效: 年化=11.73% 回撤=-34.13% 夏普=0.67 交易=11365 胜率=53.8%
...
def main():
    ...
    sig = f_double_ma_pullback(close, high=high, low=low, vol=vol, **{"fast": 10, "slow": 60})
    selections = signals_to_selections(sig, "趋势回踩10/60")
