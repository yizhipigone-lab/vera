# -*- coding: utf-8 -*-
"""自动寻优第 40 轮策略 (self-contained, 可独立运行): 放量突破
生成时间: 2026-08-11 07:10
因子: volume_breakout  入场参数: {"n": 20, "k": 2.0}
出场: 硬止损=-0.1 移动止盈激活=0.08/回撤=0.03 (条件单语义) 时间=20天 阶梯=None
仓位: 最多持 20 只, 单票 3000~15000元
手续费/滑点: 0.0003 / 0.001
区间: 20190101 ~ 20260731  股票池: hs300 (type=23, 剔除ST)
绩效: 年化=10.72% 回撤=-14.03% 夏普=0.75 交易=11070 胜率=65.7%
...
def main():
    ...
    sig = f_volume_breakout(close, high=high, low=low, vol=vol, **{"n": 20, "k": 2.0})
    selections = signals_to_selections(sig, "放量突破")
