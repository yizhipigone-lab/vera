# -*- coding: utf-8 -*-
"""自动寻优第 56 轮策略 (self-contained, 可独立运行): 动量60日15%
生成时间: 2026-08-11 07:20
因子: momentum  入场参数: {"n": 60, "thr": 0.15}
出场: 硬止损=-0.1 移动止盈激活=0.08/回撤=0.03 (条件单语义) 时间=20天 阶梯=None
仓位: 最多持 20 只, 单票 3000~15000元
手续费/滑点: 0.0003 / 0.001
区间: 20190101 ~ 20260731  股票池: hs300 (type=23, 剔除ST)
绩效: 年化=-3.94% 回撤=-54.11% 夏普=-0.11 交易=75916 胜率=49.3%
...
def main():
    ...
    sig = f_momentum(close, high=high, low=low, vol=vol, **{"n": 60, "thr": 0.15})
    selections = signals_to_selections(sig, "动量60日15%")
