...
2. **计划期估的三个数偏高**：六特征全非空的日频观测 2579 → 实测 **2461**；排除 252 天后的可选池 2327 → 实测 **2210**；`breadth50` 建仓 181 次 / 在场 47.9% / 毛 +0.6% → 实测 **179 次 / 47.1% / +0.3%**。三处**结论方向都没变**，数字已按交付代码改正（见计划书 §19.3）。
...
- 估值维度（ERP）、牛熊区间与时长、维度体检、regime 背景带与箱线图 → 归入 M3
