...
- **时间止损口径对齐（同日用户拍板第二刀，动交易行为）**：`trade_main.py` 的 `_hold_days_for_code`（监控器时间止损/条件时间/首日三条规则的持仓天数）原用「有史以来首笔买入」`MIN(ts)`——清仓后重新买入的票会被按老买入日**提前误卖**。改为当轮首笔买入：倒推逻辑纯函数化下沉 `trade/analysis.py::episode_entry_ts`，与移动止盈峰值（原 `_episode_entry_ts`）**共用同一实现**（同一条规则不写第二份），`trade_main` 两处只剩取数装配。原 2026-08-06「多算天数方向安全，不动」的拍板作废。
- **测试（TDD，先红后绿）**：`tests/trade/test_analysis.py` 新增 3 例——两轮切分（518880 复刻：summary=最新一轮/持仓 1 天/closed 两条）、清仓再买入的持仓票入场时间=新一轮首笔、同轮内多次买入不拆轮；episode_entry_ts 3 例（清仓再买→新一轮起点/部分卖出→仍最早买入/遗产仓倒推不完→回退最早+空表→None）。3 个既有锁（159290 遗产仓、表内平仓、无 pnl 历史行）原样全绿。全量 `pytest tests/`：**基线（stash 对照）52 failed / 3204 passed → 改后 52 failed / 3207 passed**，失败集合逐名相同（test_rotation/test_rotation_lots/test_notifier 等既有红，均不调用本函数），零新增。
...
- **生效条件**：改的是 `trade/analysis.py` + `trade_main.py`，**热重载端点 `reload_view` 只重载 `view_calc` 不覆盖它们 → 需重启 trade_main（8081）**，重启遵守既有纪律（不在 14:54~15:00 窗口、先等 QMT 就绪）。用户拍板当天先不重启。重启前：页面那行数字仍是旧错口径（不影响交易）；时间止损仍是旧口径（影响交易行为，重启后才按新一轮算）。
