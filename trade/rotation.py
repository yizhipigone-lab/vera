...
from trade.events import EVENT_ROTATION, PUMPABLE_WAIT_TYPES, Event
...
from trade.rotation_feed import IndexFeed  # 2026-09-19 批次 4.5 取数降级链端出
from trade.rotation_ledger import RotationLedgerMixin  # 2026-09-19 批次 4.5 在途台账端出
...
class RotationFeature(RotationLedgerMixin):
    ...
    def __init__(self, engine, cfg_getter, store, gateway, book, monitor,
        ...
        # 2026-09-19 批次 4.5: 指数取数降级链端出 trade/rotation_feed.py
        # (原 _fetch_closes/_fetch_tdx_closes/_fetch_tencent_closes 三方法,
        #  与本类状态无关的自包含块; 调用点已改 self._feed.closes)
        self._feed = IndexFeed(gateway)
        self._book = book
    ...
    def _shadow_tick(self, today) -> None:
        ...
        **只用 QMT 快源, 不走 IndexFeed.closes 降级链** (2026-08-23 测试超时修复):
    ...
    def _compute_momentum(self, now) -> dict:
        ...
        for code in risk_legs:
            closes, src = self._feed.closes(code, need, min_bars)
            if after_close:
    ...
    def _wait_fills(self, order_ids: list[str],
        ...
        2026-09-19 批次 4.2 (架构审查 P0-4): 等待期间的 sleep 换成
        `engine.pump(PUMPABLE_WAIT_TYPES, …)` —— 等待窗口内到达的**回报类**
        事件 (委托回报/成交/报错) 就地分发, tick/对账/命令等仍留队列, 于是
        "等成交"不再让整个事件队列停摆。判据仍是查网关 (真相源), 语义不变:
        等得到就继续, 等不到照样放行。
        """
        if not order_ids:
            return
        ...
        while pending:
            ...
            if remaining <= 0:
                break
            window = min(self._wait_interval, remaining)
            if self._engine.can_pump():
                # 就地消费回报类事件 (等于把这段等待"让给"队列)
                self._engine.pump(PUMPABLE_WAIT_TYPES, window)
            else:
                _time.sleep(window)   # 非消费者线程 (单测/工具): 保持原行为
